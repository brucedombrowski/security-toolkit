...
- **Fix `grep -c ... || echo "0"` producing "0\n0" on zero matches** — `grep -c` prints `0` *and* exits 1 when nothing matches, so the fallback appended a second `0`. In `check-host-security.sh` this made `[ "$UPDATE_COUNT" -eq 0 ]` fail with "integer expression expected" and report FAIL (security updates available) when `softwareupdate -l` listed only Command Line Tools. Replaced the anti-pattern with `|| true` + `${VAR:-0}` across 13 scripts and 2 tests; added a zero-match regression test and wired `test-host-security.sh` into `run-all-tests.sh`
